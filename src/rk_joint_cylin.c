...
/* CRB method */
static void _rkJointCylinCRBWrench(void *prp, rkMP *crb, zVec6D wi[]){
  _rkJointCRBWrenchLinZ( crb, &wi[0] );
  _rkJointCRBWrenchAngZ( crb, &wi[1] );
}
static void _rkJointCylinCRBXform(void *prp, zFrame3D *f, zVec6D si[]){
...
static void _rkJointCylinSetFriction(void *prp, double *val)
{
  _rkc(prp)->tf[0] = val[0];
  _rkc(prp)->tf[1] = val[1];
}
static void _rkJointCylinGetFriction(void *prp, double *val){
  val[0] = _rkc(prp)->tf[0];
  val[1] = _rkc(prp)->tf[1];
}
static void _rkJointCylinGetSFriction(void *prp, double *val){
  val[0] = _rkc(prp)->sf[0];
  val[1] = _rkc(prp)->sf[1];
}
static void _rkJointCylinGetKFriction(void *prp, double *val){
