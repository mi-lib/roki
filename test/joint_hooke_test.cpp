...
void truth(zVec dis, zVec vel, zVec acc, zFrame3D &f, zVec6D &v, zVec6D &a)
{
  ...
  // frame
  zFrame3DAtt(&f)->create(
    c1*c2,-s1, c1*s2,
    s1*c2, c1, s1*s2,
      -s2,  0,    c2 );
  *zFrame3DPos(&f) = *zFrame3DAtt(&f) * p;
  // velocity
  dq1 = zVecElem(vel,0);
  dq2 = zVecElem(vel,1);
  w.create( -dq2*s1, dq2*c1, dq1 );
  *zVec6DAng(&v) = zFrame3DAtt(&f)->mulT( w );
  zVec3DOuterProd( zVec6DAng(&v), &p, zVec6DLin(&v) );
  // acceleration
  ddq1 = zVecElem(acc,0);
  ddq2 = zVecElem(acc,1);
  dw.create( -ddq2*s1-dq1*dq2*c1, ddq2*c1-dq1*dq2*s1, ddq1 );
  *zVec6DAng(&a) = zFrame3DAtt(&f)->mulT( dw );

  zVec3DOuterProd( zVec6DAng(&v), zVec6DLin(&v), zVec6DLin(&a) );
  zVec3DOuterProd( zVec6DAng(&a), &p, &tmp );
  *zVec6DLin(&a) += tmp;
  tmp = zFrame3DAtt(&f)->mulT( *RK_GRAVITY3D );
  *zVec6DLin(&a) += tmp;
}
...
int main(void)
{
  ...
  // create wrench
  w.create( zRandF(-1,1), zRandF(-1,1), zRandF(-1,1), zRandF(-1,1), zRandF(-1,1), zRandF(-1,1) );
