...
void create_body(rkBody *b, double x, double y, double yaw)
{
  ...
  zMat3DRotYaw( ZMAT3DIDENT, yaw, rkBodyAtt(b) );
}
...
int main(int argc, char *argv[])
{
  ...
  rkMPPrint( rkBodyMP(&src[0]) );
  rkMPPrint( rkBodyMP(&src[1]) );
  rkMPPrint( rkBodyMP(&src[2]) );
  rkMPPrint( rkBodyMP(&src[3]) );
